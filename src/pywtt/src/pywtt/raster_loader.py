...
from cppwtt import minigraph as mg
import cppwtt as cw
...
def load_wtt(fname):
	'''
		Short function to locally load a minigraph from file name
	'''

	out = load_raster(fname)

	return mg(out["nx"], out["ny"], out["nx"] * out["ny"], out['dx'], out['dy'], out["x_min"], out["y_min"], out["array"])

def load_dem_to_mgraph(fname):
	'''
		Short function to locally load a minigraph from file name
	'''
